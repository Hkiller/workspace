    ...
    ui_runtime_render_program_buildin_t program, ui_runtime_render_blend_t blend)
{
    ...
    cmd = ui_runtime_render_cmd_triangle_create(
        render,
        logic_z,
        material,
        ui_runtime_render_buff_inline(vertexs, ui_runtime_render_buff_vertex_v3f_t2f_c4b, 3),
        transform, 0);
    if (cmd == NULL) {
        ui_runtime_render_material_free(material);
        return NULL;
    ...
    ui_runtime_render_program_buildin_t program, ui_runtime_render_blend_t blend)
{
    ...
    if (*cmd) {
        if (!runtime_render_cmd_state_and_texture_compatible(*cmd, logic_z, transform, texture, filter, program, blend)) {
            if (ui_runtime_render_cmd_triangle_batch_commit(cmd, render) != 0) return -1;
        }
    }
    ...
    if (buf->m_inline.m_buf == NULL) {
        void * inline_data_buf = ui_runtime_render_alloc_buf(render, ui_runtime_render_buff_vertex_v3f_t2f_c4b, 3);
        ...
        memcpy(inline_data_buf, vertexs, sizeof(vertexs[0]) * 3);
        
        *buf = ui_runtime_render_buff_inline(inline_data_buf, ui_runtime_render_buff_vertex_v3f_t2f_c4b, 3);
    }
    else {
        void * inline_data_buf = ui_runtime_render_append_buf(render, buf, 3);
...
int ui_runtime_render_cmd_triangle_batch_commit(ui_runtime_render_cmd_t * cmd, ui_runtime_render_t render) {
    uint16_t * index_buf;
    uint16_t * wp;
    ...
    /*为triangle命令补充index */
    triangle_count = ui_runtime_render_cmd_vertex_count(*cmd);
    assert(triangle_count % 3 == 0);
    triangle_count /= 3;

    index_buf = ui_runtime_render_alloc_buf(render, ui_runtime_render_buff_index_uint16, triangle_count * 3);
    if (index_buf == NULL) {
        CPE_ERROR(
            render->m_module->m_em, "ui_runtime_render_cmd_triangle_batch_commit: alloc trianglee buf fail, count=%d", triangle_count);
        ui_runtime_render_cmd_free(*cmd);
        *cmd = NULL;
        return -1;
    }
    ...
    (*cmd)->m_index_buf = ui_runtime_render_buff_inline(index_buf, ui_runtime_render_buff_index_uint16, triangle_count * 3);
    *cmd = NULL;
