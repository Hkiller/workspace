...
static int appsvr_haoxin_do_suspend(void * ctx, LPDRMETA req_meta, void const * req_data, size_t req_size) {
    appsvr_haoxin_module_t module = (appsvr_haoxin_module_t)ctx;
    APP_ENV_SUSPEND const * req = req_data; 

    if (req->is_suspend) {
        appsvr_haoxin_on_suspend(module);
    }
    else {
        if (module->m_payment_runing) {
            CPE_ERROR(module->m_em, "appsvr_haoxin_module_start_payment_cancel_timer: enter!");

            appsvr_haoxin_module_start_payment_cancel_timer(module);
        }
        appsvr_haoxin_on_resume(module);
    }
...
int appsvr_haoxin_suspend_monitor_init(appsvr_haoxin_module_t module) {
    module->m_suspend_monitor = 
        ...
        module->m_app_env,
        "app_env_suspend",
        module, appsvr_haoxin_do_suspend, NULL);
    if (module->m_suspend_monitor == NULL) {
        CPE_ERROR(module->m_em, "appsvr_haoxin_plugin_init: create suspend fail!");
        return -1;
    }
