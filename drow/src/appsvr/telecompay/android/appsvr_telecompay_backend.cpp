...
static struct {
    const char * name; 
    int (*init)(appsvr_telecompay_backend_t backend);
    void (*fini)(appsvr_telecompay_backend_t backend);
} s_auto_reg_products[] = {
    { "jni", appsvr_telecompay_jni_init, appsvr_telecompay_jni_fini }
    , { "payment", appsvr_telecompay_payment_init, appsvr_telecompay_payment_fini }
};

int appsvr_telecompay_backend_init(appsvr_telecompay_module_t module) {
    appsvr_telecompay_backend_t backend;
    uint16_t component_pos;
    
    backend = (appsvr_telecompay_backend_t)mem_alloc(module->m_alloc, sizeof(struct appsvr_telecompay_backend));
    if (backend == NULL) {
        CPE_ERROR(module->m_em, "appsvr_telecompay_backend_create: alloc fail!");
        return -1;
    }
    ...
    CPE_INFO(module->m_em, "appsvr_telecompay_backend_create: success!");
...
int appsvr_telecompay_show_stop_page(appsvr_telecompay_module_t module) {
    ...
    if (env->ExceptionCheck()) {
        CPE_ERROR(module->m_em, "appsvr_telecompay_do_stop: call  fail!");
...
int appsvr_telecompay_show_more_game_page(appsvr_telecompay_module_t module) {
    ...
    if (env->ExceptionCheck()) {
        CPE_ERROR(module->m_em, "appsvr_telecompay_do_more_games: call  fail!");
        env->ExceptionDescribe();
        env->ExceptionClear();
        return -1;
...
extern "C" {
    ...
    JNIEXPORT void JNICALL Java_com_drowgames_telecompay_TelecomOffLineExitGameListener_nativeNotifyResult(
