...
static void appsvr_chuangku_on_activity_result(void * ctx, void * intent, int requestCode, int resultCode) {
    ...
    if (env->ExceptionCheck()) {
        CPE_ERROR(module->m_em, "appsvr_chuangku_on_activity_result: call loing fail!");
        env->ExceptionDescribe();
        env->ExceptionClear();
        return;
    }
    CPE_ERROR(module->m_em, "appsvr_chuangku_on_activity_result: enter!");
}

static void appsvr_chuangku_on_new_intent(void * ctx, void * intent) {
    appsvr_chuangku_module_t module = (appsvr_chuangku_module_t)ctx;
    ...
    if (env->ExceptionCheck()) {
        CPE_ERROR(module->m_em, "appsvr_chuangku_on_new_intent: call loing fail!");
        env->ExceptionDescribe();
        env->ExceptionClear();
        return;
    }
    CPE_ERROR(module->m_em, "appsvr_chuangku_on_new_intent: enter!");
}
...
int appsvr_chuangku_delegate_init(appsvr_chuangku_backend_t backend) {
    if (plugin_app_env_android_register_delegate(
            ...
            backend->m_module,
            appsvr_chuangku_on_new_intent,
            appsvr_chuangku_on_activity_result) != 0)
    {
        CPE_ERROR(backend->m_module->m_em, "appsvr_chuangku_delegate_init: register fail!");
        return -1;
    }
