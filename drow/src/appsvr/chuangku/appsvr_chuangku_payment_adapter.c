...
static void appsvr_chuangku_module_clear_payment_cancel_timer(appsvr_chuangku_module_t module);

static int appsvr_chuangku_payment_pay_start(appsvr_payment_adapter_t adapter, APPSVR_PAYMENT_BUY const * req) {
    appsvr_chuangku_module_t module = *(appsvr_chuangku_module_t*)appsvr_payment_adapter_data(adapter);
    int rv;

    if (module->m_payment_runing) {
        CPE_ERROR(module->m_em, "appsvr_chuangku_payment_pay_start: call fail!");
        return -1;
    }

    rv = appsvr_chuangku_backend_pay_start(module, req);
    if (rv==0) {
...
void appsvr_chuangku_module_send_payment_result(appsvr_chuangku_module_t module, APPSVR_PAYMENT_RESULT const * payment_result) {
    if (!module->m_payment_runing) {
        CPE_ERROR(module->m_em, "appsvr_chuangku_module: send payment result: no payment runing!");
        return;
    }

    appsvr_chuangku_module_clear_payment_cancel_timer(module); /*取消保护的定时器 */
    module->m_payment_runing = 0;
    
    appsvr_payment_adapter_notify_result(module->m_payment_adapter, payment_result);  
}

static void appsvr_chuangku_module_on_runing_cancel(void * ctx, cpe_timer_id_t timer_id, void * arg) {
    appsvr_chuangku_module_t module = ctx;
    CPE_ERROR(module->m_em, "appsvr_chuangku_module_on_runing_cancel: enter!");
    ...
    if (module->m_payment_runing) {
        ...
        appsvr_payment_adapter_notify_result(module->m_payment_adapter, &payment_result);
        CPE_ERROR(module->m_em, "appsvr_chuangku_module_on_runing_cancel: enter success!");
    }
}
...
int appsvr_chuangku_module_start_payment_cancel_timer(appsvr_chuangku_module_t module) {
    ...
    if (timer_mgr == NULL) {
        CPE_ERROR(module->m_em, "appsvr_chuangku_module: start runint cancel timer: no timer manager!");
        return -1;
    }
    ...
    if (gd_timer_mgr_regist_timer(
            ...
            appsvr_chuangku_module_on_runing_cancel, module,
            NULL, NULL, 5000, 0, 1)
        != 0)
    {
        CPE_ERROR(module->m_em, "appsvr_chuangku_module: start runint cancel timer fail!");
        return -1;
    }

    CPE_ERROR(module->m_em, "appsvr_chuangku_module_start_payment_cancel_timer: enter success!");

    return 0;
}

static void appsvr_chuangku_module_on_runing_success(void * ctx, cpe_timer_id_t timer_id, void * arg) {
    ...
    if (module->m_payment_runing) {
        ...
        cpe_str_dup(payment_result.error_msg, sizeof(payment_result.error_msg), "");
        appsvr_payment_adapter_notify_result(module->m_payment_adapter, &payment_result);
        CPE_ERROR(module->m_em, "appsvr_chuangku_module_on_runing_success: enter success!");
    }
}
...
int appsvr_chuangku_module_start_payment_success_timer(appsvr_chuangku_module_t module) {
    ...
    if (gd_timer_mgr_regist_timer(
        ...
        appsvr_chuangku_module_on_runing_success, module,
        NULL, NULL, 5000, 0, 1)
        != 0)
    {
        CPE_ERROR(module->m_em, "appsvr_chuangku_module: start runint success timer fail!");
        return -1;
    }

    CPE_ERROR(module->m_em, "appsvr_chuangku_module_start_payment_success_timer: enter success!");
...
int appsvr_chuangku_payment_init(appsvr_chuangku_module_t module) {
	module->m_payment_adapter = 
        ...
        appsvr_payment_service_chuangku_offline, "chuangku",
        0, 0,
		sizeof(appsvr_chuangku_module_t), appsvr_chuangku_payment_pay_start, NULL);
	if (module->m_payment_adapter == NULL) {
		CPE_ERROR(module->m_em, "appsvr_chuangku_login_init: create adapter fail!");
		return -1;
	}

	*(appsvr_chuangku_module_t*)appsvr_payment_adapter_data(module->m_payment_adapter) = module;
...
void appsvr_chuangku_payment_fini(appsvr_chuangku_module_t module) {
    ...
    appsvr_chuangku_module_clear_payment_cancel_timer(module);
}
