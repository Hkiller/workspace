...
static void appsvr_facebook_share_on_activity_result(void * ctx, void * intent, int requestCode, int resultCode) {
    appsvr_facebook_share_module_t module = (appsvr_facebook_share_module_t)ctx;
    ...
    if (env->ExceptionCheck()) {
        CPE_ERROR(module->m_em, "appsvr_facebook_share_on_activity_result: call loing fail!");
        env->ExceptionDescribe();
        env->ExceptionClear();
        return;
    }
    CPE_ERROR(module->m_em, "appsvr_facebook_share_on_activity_result: enter!");
}
...
int appsvr_facebook_share_delegate_init(appsvr_facebook_share_backend_t backend) {
    if (plugin_app_env_android_register_delegate(
            ...
            backend->m_module,
            NULL,
            appsvr_facebook_share_on_activity_result) != 0)
    {
        CPE_ERROR(backend->m_module->m_em, "appsvr_facebook_share_delegate_init: register fail!");
        return -1;
    }
