...
ui_sprite_chipmunk_touch_trace_t
ui_sprite_chipmunk_touch_trace_create(ui_sprite_chipmunk_env_t env, int32_t id, ui_vector_2_t last_pt) {
    ui_sprite_chipmunk_touch_trace_t trace;

    trace = (ui_sprite_chipmunk_touch_trace_t)mem_alloc(env->m_module->m_alloc, sizeof(struct ui_sprite_chipmunk_touch_trace));
    if (trace == NULL) {
        CPE_ERROR(env->m_module->m_em, "alloc chipmunk_touch trace fail!");
        return NULL;
    }

    trace->m_id = id;
    trace->m_is_ending = 0;
    trace->m_last_pt = *last_pt;
...
void ui_sprite_chipmunk_touch_trace_free(ui_sprite_chipmunk_env_t env, ui_sprite_chipmunk_touch_trace_t trace) {
    ...
    while(!TAILQ_EMPTY(&trace->m_bindings)) {
        ui_sprite_chipmunk_touch_binding_free(TAILQ_FIRST(&trace->m_bindings));
    }

    mem_free(env->m_module->m_alloc, trace);
}

void ui_sprite_chipmunk_touch_trace_free_all(ui_sprite_chipmunk_env_t env) {
    while(!TAILQ_EMPTY(&env->m_touch_traces)) {
        ui_sprite_chipmunk_touch_trace_free(env, TAILQ_FIRST(&env->m_touch_traces));
    }
}
