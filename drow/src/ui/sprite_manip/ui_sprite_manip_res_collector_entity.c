...
static int plugin_package_manip_res_collector_entity_do(
    void * ctx, plugin_package_package_t package, cfg_t res_cfg, cfg_t package_cfg, cfg_calc_context_t args)
{
    ...
    const char * path_def;
    cfg_t entity_cfg;
    cfg_calc_context_t last_arg = args;
    ...
    if (path_def) {
        ...
        if ((path = plugin_package_manip_calc(module->m_package_manip, &buffer, path_def, args)) == NULL) {
            CPE_ERROR(
                module->m_em, "package: %s: entity collect: calc from %s fail",
        ...
        if (path[0] == '^') {
            entity_cfg = cfg_find_cfg(gd_app_cfg(module->m_app), path + 1);
        }
        else {
            entity_cfg = cfg_find_cfg(last_arg->m_cfg, path);
        }
        
        mem_buffer_clear(&buffer);
    }
    else {
        entity_cfg = last_arg->m_cfg;
    }

    return ui_app_manip_collect_src_from_entity(
        plugin_package_package_srcs(package),
        plugin_package_package_resources(package),
        entity_cfg, module->m_em);
}

static int plugin_package_manip_res_collector_entity(void * ctx, plugin_package_package_t package, cfg_t res_cfg, cfg_t package_cfg) {
    ui_sprite_manip_t module = ctx;
    ...
    if ((search = cfg_get_string(res_cfg, "search", NULL))) {
        ...
        rv = plugin_package_manip_collect_search(
            ...
            search_path, plugin_package_manip_res_collector_entity_do, manip);
        ...
        return plugin_package_manip_res_collector_entity_do(ctx, package, res_cfg, package_cfg, &calc_ctx);
    }
}
...
int ui_sprite_manip_res_collector_entity_regist(ui_sprite_manip_t module) {
    plugin_package_manip_res_collector_t collector =
        plugin_package_manip_res_collector_create(
            module->m_package_manip, "entity", plugin_package_manip_res_collector_entity, module);
    if (collector == NULL) return -1;
    return 0;
}

void  ui_sprite_manip_res_collector_entity_unregist(ui_sprite_manip_t module) {
    plugin_package_manip_res_collector_t collector = plugin_package_manip_res_collector_find(module->m_package_manip, "entity");
    if (collector) {
