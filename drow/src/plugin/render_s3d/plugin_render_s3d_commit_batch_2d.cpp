...
void plugin_render_s3d_batch_2d_queue_cmd(plugin_render_s3d_module_t module, ui_runtime_render_cmd_t cmd) {
    plugin_render_s3d_batch_2d_t batch_2d = module->m_batch_2d;
    ...
    if (batch_2d->m_queued_triangle_command_count >= batch_2d->m_queued_triangle_command_capacity) {
        ...
        new_buf = (ui_runtime_render_cmd_t *)mem_alloc(module->m_alloc, sizeof(ui_runtime_render_cmd_t) * new_capacity);
        if (new_buf == NULL) {
            CPE_ERROR(module->m_em, "plugin_render_s3d_batch_2d_queue_cmd: alloc buf fail, capacity=%d", new_capacity);
            return;
        }
...
static int plugin_render_s3d_batch_2d_fill_vertices_and_indices(
    plugin_render_s3d_module_t module, plugin_render_s3d_batch_2d_t batch_2d, ui_runtime_render_cmd_t cmd)
{
    ...
    uint16_t * indexes = batch_2d->m_indexes + batch_2d->m_index_count;
    const uint16_t * input_indexes = (const uint16_t * )ui_runtime_render_cmd_index_data(cmd);
    ...
    case ui_runtime_render_buff_vertex_v3f_t2f_c4b:
        ...
        /*调整uv坐标系 */
        //for(i = 0; i < vertex_count; ++i) { vertex[i].m_uv.y = 1.0f - vertex[i].m_uv.y; }
        break;
    default:
        CPE_ERROR(
            module->m_em, "plugin_render_s3d_batch_2d_fill_vertices_and_indices: not support index type %s",
    ...
    default:
        CPE_ERROR(
            module->m_em, "plugin_render_s3d_batch_2d_fill_vertices_and_indices: not support element type %s",
...
static plugin_render_s3d_batch_cmd_t
plugin_render_s3d_batch_2d_command_append(plugin_render_s3d_module_t module, plugin_render_s3d_batch_2d_t batch_2d, ui_runtime_render_cmd_t cmd) {
    plugin_render_s3d_batch_cmd_t batch_cmd;
    ...
    if (batch_2d->m_bached_command_count >= batch_2d->m_bached_command_capacity) {
        ...
        void * new_buf = mem_alloc(module->m_alloc, sizeof(struct plugin_render_s3d_batch_cmd) * new_capacity);

        if (new_buf) {
            CPE_ERROR(
                module->m_em, "plugin_render_s3d_batch_2d_init: alloc batch commands fail, capacity=%d",
                new_capacity);
            return NULL;
        }
        ...
        if (batch_2d->m_bached_commands) {
            ...
            memcpy(new_buf, batch_2d->m_bached_commands, sizeof(struct plugin_render_s3d_batch_cmd) * batch_2d->m_bached_command_count);
            mem_free(module->m_alloc, batch_2d->m_bached_commands);
        }

        batch_2d->m_bached_commands = (plugin_render_s3d_batch_cmd_t)new_buf;
...
static uint8_t plugin_render_s3d_batch_2d_batchable(ui_runtime_render_cmd_t cmd) {
...
static uint8_t plugin_render_s3d_batch_2d_cmd_compact(ui_runtime_render_cmd_t pre, ui_runtime_render_cmd_t check) {
...
void plugin_render_s3d_batch_2d_commit(plugin_render_s3d_module_t module) {
    plugin_render_s3d_batch_2d_t batch_2d = module->m_batch_2d;
    uint32_t i;
    plugin_render_s3d_batch_cmd_t batch_command = NULL;
    ...
    for(i = 0; i < batch_2d->m_queued_triangle_command_count; ++i) {
        ...
        uint8_t batchable = plugin_render_s3d_batch_2d_batchable(cmd);

        if (plugin_render_s3d_batch_2d_fill_vertices_and_indices(module, batch_2d, cmd) != 0) continue;

        if (batch_command && batchable && plugin_render_s3d_batch_2d_cmd_compact(batch_command->m_cmd, cmd)) {
            /*可以合并 */
            ...
            /*不可以合并 */
            batch_command = plugin_render_s3d_batch_2d_command_append(module, batch_2d, cmd);
        }
    ...
    /*拷贝数据 */
    /*    顶点数据 */
    flash::display3D::VertexBuffer3D v_buf;
    try {
        v_buf =
            module->m_ctx3d->createVertexBuffer(
                batch_2d->m_vertex_count,
                sizeof(struct ui_runtime_vertex_v3f_t2f_c4ub) / sizeof(float));
        
        v_buf->uploadFromByteArray(
            internal::get_ram(), (int)&batch_2d->m_vertexes[0], 0, batch_2d->m_vertex_count, (void*)&batch_2d->m_vertexes[0]);
    }
    S3D_REPORT_EXCEPTION("2d_commit: upload vertex buf", );

    /*    索引数据 */
    flash::display3D::IndexBuffer3D i_buf;
    try {
        i_buf = module->m_ctx3d->createIndexBuffer(batch_2d->m_index_count);
        i_buf->uploadFromByteArray(
            internal::get_ram(), (int)&batch_2d->m_indexes[0], 0, batch_2d->m_index_count, (void*)&batch_2d->m_indexes[0]);
    }
    S3D_REPORT_EXCEPTION("2d_commit: upload index buf", );

    /*绘制操作 */
    for (i = 0; i < batch_2d->m_bached_command_count; ++i) {
        struct plugin_render_s3d_batch_cmd * batch_cmd = &batch_2d->m_bached_commands[i];
        ...
        while((pass = ui_runtime_render_pass_it_next(&pass_it))) {
            if (plugin_render_s3d_bind_pass(module, v_buf, pass, NULL) != 0) continue;
            ...
            try {
                module->m_ctx3d->drawTriangles(i_buf, batch_cmd->m_index_offset, batch_cmd->m_index_count / 3);
            }
            S3D_REPORT_EXCEPTION_2(
                "2d_commit: drawTriangles(%d~%d]",
                batch_cmd->m_index_offset, batch_cmd->m_index_offset + batch_cmd->m_index_count, );
...
int plugin_render_s3d_batch_2d_init(plugin_render_s3d_module_t module) {
    plugin_render_s3d_batch_2d_t batch_2d;
    
    batch_2d = (plugin_render_s3d_batch_2d_t)mem_alloc(
        module->m_alloc,
        sizeof(struct plugin_render_s3d_batch_2d)
        + sizeof(struct ui_runtime_vertex_v3f_t2f_c4ub) * module->m_capacity_vertex_size
        + sizeof(uint16_t) * module->m_capacity_index_size);
    if (batch_2d == NULL) {
        CPE_ERROR(
            module->m_em, "plugin_render_s3d_batch_2d_init: alloc fail, vertex_size=%d, index_size=%d",
            module->m_capacity_vertex_size, module->m_capacity_index_size);
        return -1;
    }

    batch_2d->m_vertex_count = 0;
    batch_2d->m_vertexes = (ui_runtime_vertex_v3f_t2f_c4ub_t)(batch_2d + 1);
    batch_2d->m_index_count = 0;
    batch_2d->m_indexes = (uint16_t*)(batch_2d->m_vertexes + module->m_capacity_vertex_size);
    ...
    batch_2d->m_bached_commands =
        (plugin_render_s3d_batch_cmd_t)mem_alloc(
            module->m_alloc, sizeof(struct plugin_render_s3d_batch_cmd) * batch_2d->m_bached_command_capacity);
    ...
    assert((sizeof(struct ui_runtime_vertex_v3f_t2f_c4ub) % sizeof(float)) == 0);
