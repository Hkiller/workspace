...
struct ui_sprite_control_data_find_ctx {
...
xtoken_t plugin_ui_control_find_value(void * input_ctx, xcomputer_t computer, const char * attr_name, error_monitor_t em) {
    struct ui_sprite_control_data_find_ctx * ctx = input_ctx;
    ...
    while (attr_name[0] == '[') {
        char name_buf[64];
        const char * control_name = cpe_str_trim_head((char*)attr_name + 1);
        const char * control_name_end = strchr(control_name, ']');
        size_t control_name_len;

        if (control_name_end == NULL) {
            CPE_ERROR(em, "plugin_ui_control_find_value: %s format error!", attr_name);
            return NULL;
        }

        control_name_len = control_name_end - control_name;
        if ((control_name_len + 1) > CPE_ARRAY_SIZE(name_buf)) {
            CPE_ERROR(em, "plugin_ui_control_find_value: control name len %d overflow!", (int)control_name_len);
            return NULL;
        }

        memcpy(name_buf, control_name, control_name_len);
        name_buf[control_name_len] = 0;

        if (name_buf[0] == '@') {
            xtoken_t v = plugin_ui_control_find_value(input_ctx, computer, name_buf + 1, em);
            if (v == NULL) {
                CPE_ERROR(em, "plugin_ui_control_find_value: %s: calc inner arg %s fail!", attr_name, name_buf + 1);
                return NULL;
            }
            ...
            case xtoken_data_str: {
                ...
                if (ctx->m_page == NULL) {
                    CPE_ERROR(em, "plugin_ui_control_find_value: %s: from arg %s: page %s not exist!", attr_name, name_buf + 1, str_v);
                    xcomputer_free_token(computer, v);
                    return NULL;
                }
                break;
            ...
            default:
                CPE_ERROR(
                    em, "plugin_ui_control_find_value: %s: calc inner arg %s: result type %d fail!",
            ...
            if (ctx->m_control == NULL) {
                CPE_ERROR(em, "plugin_ui_control_find_value: page %s not exist!", name_buf);
                return NULL;
            }
        }

        attr_name = cpe_str_trim_head((char*)control_name_end + 1);
    }
    ...
    if ((attr_meta = plugin_ui_control_attr_meta_find(ctx->m_control->m_meta, attr_name))) {
        struct dr_value attr_value;
        
        if (attr_meta->m_getter == NULL) {
            CPE_ERROR(
                em, "plugin_ui_control_find_value: control %s attr %s can`t read!",
                ui_data_control_type_name(ctx->m_control->m_meta->m_type), attr_name);
            return NULL;
        }
        
        if (attr_meta->m_getter(ctx->m_control, &attr_value) != 0) {
            CPE_ERROR(
                em, "plugin_ui_control_find_value: control %s attr %s read fail!",
                ui_data_control_type_name(ctx->m_control->m_meta->m_type), attr_name);
            return NULL;
        }

        return dr_create_token_from_value(computer, &attr_value);
    }
    
    CPE_ERROR(em, "plugin_ui_control_find_value: %s: entry not exist!", attr_name);
    return NULL;
}

xtoken_t plugin_ui_control_calc_i(plugin_ui_control_t control, const char * def, dr_data_source_t data_source) {
    struct ui_sprite_control_data_find_ctx ctx = { control->m_page->m_env, control->m_page, control, data_source };
    struct xcomputer_args calc_args = { &ctx, plugin_ui_control_find_value };
    return xcomputer_compute(control->m_page->m_env->m_module->m_computer, def, &calc_args);
}
...
uint8_t plugin_ui_control_calc_bool_with_dft(const char * def, plugin_ui_control_t control, dr_data_source_t data_source, int8_t dft) {
    struct ui_sprite_control_data_find_ctx ctx = { control->m_page->m_env, control->m_page, control, data_source };
    struct xcomputer_args calc_args = { &ctx, plugin_ui_control_find_value };
    xcomputer_t computer = control->m_page->m_env->m_module->m_computer;
    xtoken_t value;
    uint8_t r;
...
int plugin_ui_control_try_calc_int32(int32_t * result, const char * def, plugin_ui_control_t control, dr_data_source_t data_source, error_monitor_t em) {
    struct ui_sprite_control_data_find_ctx ctx = { control->m_page->m_env, control->m_page, control, data_source };
    struct xcomputer_args calc_args = { &ctx, plugin_ui_control_find_value };
    xcomputer_t computer = control->m_page->m_env->m_module->m_computer;
    xtoken_t value;
