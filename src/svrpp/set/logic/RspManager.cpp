...
#include "svrpp/set/logic/RspManager.hpp"

namespace Svr { namespace Set {

RspManager & RspManager::_cast(set_logic_rsp_manage_t rsp_manage) {
    if (rsp_manage == NULL) {
        throw ::std::runtime_error("Svr::Set::RspManager::_cast: input rsp_manage is NULL!");
    }
...
RspManager & RspManager::instance(gd_app_context_t app, const char * name) {
    set_logic_rsp_manage_t rsp_manage = set_logic_rsp_manage_find_nc(app, name);
    if (rsp_manage == NULL) {
        APP_CTX_THROW_EXCEPTION(
            app,
            ::std::runtime_error,
            "set_logic_rsp_manage %s not exist!", name ? name : "default");
    }
...
bool RspManager::hasOp(const char * rspName) const {
    return set_logic_rsp_find(*this, rspName) != NULL;
}
...
void RspManager::loadRsps(cfg_t cfg, LPDRMETALIB metalib) {
    ...
    if (set_logic_rsp_build(*this, cfg, metalib, ec) != 0) {
