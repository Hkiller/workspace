...
static ringbuffer_block_t on_xml_request_buf_alloc(void * ctx) {
...
static void on_xml_request(conn_http_request_t request, const void * input_data, size_t input_data_size) {
...
RETRY:
    ...
    if (ringbuffer_block_data(svr->m_ringbuf, blk, 0, &data) < 0) {
        CPE_ERROR(
            svr->m_em, "%s: request %d at connection %d: on xml request: get ringbuf data fail!",
    ...
    data_size = dr_xml_read(data, cmd->m_pkg_buf_size, buf, cmd->m_req_meta, svr->m_em);
    if (data_size <= 0) {
        if (data_size == dr_code_error_not_enough_output) {
            uint32_t new_pkg_buf_size = cmd->m_pkg_buf_size < 64 ? 64 : cmd->m_pkg_buf_size * 2;
            
            CPE_INFO(
                svr->m_em, "%s: request %d at connection %d: on xml request: resize pkg buf from %d to %d!",
            ...
            CPE_ERROR(
                svr->m_em, "%s: request %d at connection %d: on xml request: read from xml fail, error=%d!",
    ...
    if (svr->m_debug) {
        CPE_INFO(
            svr->m_em, "%s: request %d at connection %d: read data from xml success, data-size=%d!",
            conn_http_svr_name(svr), request->m_id, connection->m_id, data_size);
    }
}

static void on_xml_response(conn_http_request_t request, const void * data, size_t data_len, LPDRMETA data_meta) {
    conn_http_connection_t connection = request->m_connection;
    conn_http_service_t service = request->m_cmd->m_service;
    conn_http_svr_t svr = service->m_svr;
    int result_size;
    struct write_stream_ringbuffer stream = CPE_WRITE_STREAM_RINGBUFFER_INITIALIZER(svr->m_ringbuf, on_xml_request_buf_alloc, (void*)request);

    /*构造body */
    result_size = dr_xml_print((write_stream_t)&stream, data, data_len, data_meta, 0, svr->m_em);
    if (stream.m_error) {
        CPE_ERROR(
            svr->m_em, "%s: request %d at connection %d: on xml response: generate result ringbuf error!",
            conn_http_svr_name(svr), request->m_id, connection->m_id);
        return;
    }
    ...
    if (result_size < 0) {
        CPE_ERROR(
            svr->m_em, "%s: request %d at connection %d: on xml response: write to xml fail, error=%d!",
            conn_http_svr_name(svr), request->m_id, connection->m_id, result_size);
        conn_http_request_set_error(request, 400, "Bad Request");
        return;
    }

    conn_http_request_set_response(request, "text/xml", stream.m_first_blk, result_size);
}

struct conn_http_formator g_conn_http_formator_xml = {
    on_xml_request
    , on_xml_response
};
