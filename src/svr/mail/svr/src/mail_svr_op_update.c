    ...
    logic_context_t ctx, logic_stack_node_t stack, void * user_data, cfg_t cfg)
{
    ...
    SVR_MAIL_REQ_UPDATE_MAIL const * req;

    req_data = logic_context_data_find(ctx, "svr_mail_req_update_mail");
    if (req_data == NULL) {
    ...
    require = logic_require_create(stack, "update_mail");
    if (require == NULL) {
    ...
    if (mail_svr_db_send_update(svr, require, req->mail_id, req->receiver_gid, req->ops, req->op_count) != 0) {
        logic_context_errno_set(ctx, SVR_MAIL_ERROR_INTERNAL);
        return logic_op_exec_result_false;
    }
    ...
    void * user_data, cfg_t cfg)
{
    ...
    if (logic_require_state(require) != logic_require_state_done) {
        if (logic_require_state(require) != logic_require_state_error) {
            APP_CTX_ERROR(
                logic_context_app(ctx), "%s: update: db request error, errno=%d!",
            ...
            APP_CTX_ERROR(
                logic_context_app(ctx), "%s: update: db request state error, state=%s!",
