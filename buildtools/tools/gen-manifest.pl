...
foreach my $merge_file ( @merge_files ) {
  ...
  if (exists $merge->{'application'}) {
    ...
    if (exists $merge_application->{provider}) {
      if (ref($merge_application->{'provider'}) eq "ARRAY") {
        foreach my $provider ( @{ $merge_application->{'provider'} } ) {
          add_provider($manifest, $provider);
        }
      }
      else {
        add_provider($manifest, $merge_application->{'provider'});
      }
    }
...
sub add_provider {
  (my $manifest, my $provider) = @_;

  my $application = $manifest->{'application'} or die "$input_file_name no application config";

  if (not exists $application->{provider}) {
    $application->{provider} = $provider;
  } elsif (ref($application->{provider}) eq "ARRAY") {
    push @{ $application->{provider} }, $provider;
  } else {
    $application->{provider} = [ $application->{provider}, $provider ];
  }
